...
class TestCriar:
    async def test_cria_ocorrencia(
...
class TestListar:
    async def test_lista_com_filtros(
        self, service: OcorrenciaService, repo: MagicMock
    ) -> None:
...
class TestBuscar:
    ...
    async def test_lanca_excecao_quando_nao_encontrado(
        ...
        with pytest.raises(OcorrenciaNaoEncontradaError):
...
class TestAtualizarStatus:
    async def test_transicao_valida(
        self, service: OcorrenciaService, repo: MagicMock
    ) -> None:
    ...
    async def test_transicao_invalida(
        self, service: OcorrenciaService, repo: MagicMock
    ) -> None:
