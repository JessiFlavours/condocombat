...
class TestCriar:
    async def test_cria_rivalidade(
    ...
    async def test_rejeita_apartamentos_iguais(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
        with pytest.raises(
            RivalidadeJaExisteError,
            match="Não é possível criar rivalidade consigo mesmo",
        ):
            await service.criar(101, 101)
        repo.create.assert_not_called()

    async def test_rejeita_rivalidade_ja_existente(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
        repo.get_between = AsyncMock(return_value=MagicMock())
        with pytest.raises(RivalidadeJaExisteError):
            await service.criar(101, 102)
        repo.create.assert_not_called()

    async def test_rejeita_nivel_invalido(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
        repo.get_between = AsyncMock(return_value=None)
...
class TestBuscar:
    async def test_retorna_quando_encontrado(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
    ...
    async def test_lanca_excecao_quando_nao_encontrado(
        ...
        with pytest.raises(RivalidadeNaoEncontradaError):
            await service.buscar(999)


class TestListarPorApartamento:
    async def test_retorna_rivalidades_do_apto(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
    ...
    async def test_retorna_vazio_quando_nenhuma(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
...
class TestTopRivalidades:
    async def test_retorna_top_rivalidades(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
...
class TestEscalar:
    ...
    async def test_nao_escala_quando_ja_no_maximo(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
...
class TestAtualizar:
    async def test_atualiza_campos(
        self, service: RivalidadeService, repo: MagicMock
    ) -> None:
        repo.get_by_id = AsyncMock(return_value=_make_rivalidade())
